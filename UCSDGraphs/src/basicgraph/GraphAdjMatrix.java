...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		List<Integer> dist2List = new ArrayList<Integer>();
		int adjMatrix2[][] = new int[adjMatrix.length][adjMatrix.length];
		adjMatrix2 = matrixMultiply(adjMatrix, adjMatrix);
		for(int i=0; i<adjMatrix2[0].length; i++)
		{
			int count = adjMatrix2[v][i];
			while(count-->0)
				dist2List.add(i);
		}
		return dist2List;
